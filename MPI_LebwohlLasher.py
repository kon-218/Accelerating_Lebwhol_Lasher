...
def savedat(arr,nsteps,Ts,runtime,ratio,energy,order,nmax):
    ...
    filename = "logs/MPI-n{:d}-LL-Output-{:s}.txt".format(comm.Get_size(),current_datetime)
    FileOut = open(filename,"w")
    # Write a header with run parameters
    print("#=====================================================",file=FileOut)
    print('# MPI Monte Carlo',file=FileOut)
    print('# Num Procs:           {:d}'.format(comm.Get_size()),file=FileOut)
    ...
    print("#=====================================================",file=FileOut)
    print("# MPI MC step:  Ratio:     Energy:   Order:",file=FileOut)
    print("#=====================================================",file=FileOut)
    # Write the columns of data
    for i in range(nsteps+1):
        print("   {:05d}    {:f} {:12.4f}  {:6.4f} ".format(i,ratio[i],energy[i],order[i]),file=FileOut)
    FileOut.close()
#=======================================================================
...
def MC_parallel_step(arr,Ts,nmax,comm):
    ...
    for i in range(start_row, end_row):
        for j in range(nmax):
            ix = i
            iy = j
            ang = np.random.normal(scale=0.1+Ts)  # precompute random number
            en0 = one_energy(arr,ix,iy,nmax)
            arr[ix,iy] += ang
            en1 = one_energy(arr,ix,iy,nmax)
            if en1<=en0:
...
#     initial = time.time()
#     for it in range(1,nsteps+1):
#         ratio[it] = MC_parallel_step(lattice,temp,nmax,comm)
#         energy[it] = all_energy(lattice,nmax)
#         order[it] = get_order(lattice,nmax)
#     final = time.time()
#     runtime = final-initial
